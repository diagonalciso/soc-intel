...
_FEED_BASE = "https://osv-vulnerabilities.storage.googleapis.com/{ecosystem}/all.zip"
    ...
    async def run(self) -> IngestResult:
        ...
        for ecosystem in _ECOSYSTEMS:
            ...
            try:
                ...
                vulns = _parse_osv_zip(resp.content)
...
def _parse_osv_zip(raw: bytes) -> list[dict]:
    """
    OSV bulk feeds switched from all.json to all.zip in 2024.
    The ZIP contains one JSON file per vulnerability (NDJSON-style, one object per file).
    """
    vulns = []
    try:
