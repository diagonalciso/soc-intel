...
LEAK_SITES = [
    ...
    # NOTE: address needs re-verification — known to rotate; scrape will fail gracefully if down
    {
        "group": "medusa",
        "display": "Medusa",
        "url": "http://medusaxko7ul4aum5ei2iceybog37j5a3m3okh3kg4suupwxzzdwqadf.onion",
        "parser": "generic",
    },
...
class LeakSiteScraper(BaseConnector):
    ...
    async def _register_sites(self):
        """Insert DarkWebForum records for each known site (create-only, preserves original created)."""
        ...
        for site in LEAK_SITES:
            ...
            # Use "create" so existing docs (and their created timestamp) are never overwritten
            bulk.append({"create": {"_index": DARKWEB_INDEX, "_id": doc_id}})
            bulk.append(doc)
        if bulk:
            try:
                resp = await client.bulk(body=bulk, refresh=False)
                # 409 conflicts = already exists, not errors
                real_errors = [
                    item for item in resp["items"]
                    if item.get("create", {}).get("error") and
                    item["create"]["error"].get("type") != "version_conflict_engine_exception"
                ]
                if real_errors:
                    logger.warning(f"LeakSites: {len(real_errors)} site registration errors")
    ...
    async def _store_victims(self, group: str, victims: list[dict]) -> IngestResult:
        ...
        for v in victims:
            ...
            # "create" preserves first-seen timestamp; victims already scraped are not re-stamped
            bulk.append({"create": {"_index": DARKWEB_INDEX, "_id": doc_id}})
            bulk.append(doc)
