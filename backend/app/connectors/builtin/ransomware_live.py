...
class RansomwareLiveConnector(BaseConnector):
    ...
    async def _ingest_victims(self) -> IngestResult:
        ...
        # Try v2 with extended timeout first, fall back to v1 (API can be slow, needs 90s)
        async with httpx.AsyncClient(timeout=90.0, follow_redirects=True) as slow_client:
            for url in (f"{self.BASE_URL}/recentvictims", f"{self.BASE_URL_V1}/recentvictims"):
                try:
                    resp = await slow_client.get(url, headers=headers)
