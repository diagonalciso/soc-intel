...
export default function CampaignsPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () =>
      getCampaigns({ q: search || undefined, size: 200 }).then((r) => r.data),
    placeholderData: (prev: unknown) => prev,
  })

  const campaigns: Campaign[] = (data?.objects || []).slice().sort((a: Campaign, b: Campaign) => {
    const aDate = a.first_seen || a.created || ''
    const bDate = b.first_seen || b.created || ''
